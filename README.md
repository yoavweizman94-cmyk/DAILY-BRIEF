...
`APPROVAL_SECRET`, `SESSION_SECRET`, `OWNER_EMAIL`, `FROM_EMAIL`, וקישור
KV בשם `USERS`. `SESSION_SECRET` ו-`APPROVAL_SECRET` נוצרים אוטומטית
ע"י workflow `auth-setup`.
...
`register.js` יוצרת חשבון במצב `pending` עם הסיסמה שהמשתמש בחר, כבר
מגובבת, ושולחת לך מייל עם קישור חתום. `approve.js` מפעילה את החשבון רק
בלחיצה על הקישור — מילוי הטופס לבדו לעולם אינו מעניק גישה. `login.js`
מאמתת ומנפיקה עוגיית סשן חתומה, ו-`_middleware.js` חוסם כל נתיב באפליקציה
בלי סשן תקף, **בכשל סגור**: חוסר הגדרה או חריגה מפנים לדף הכניסה.

הסיסמאות בגיבוב PBKDF2-SHA256 עם מלח אקראי לכל משתמש. Workers חוסם מעל
100,000 סבבים בקריאה אחת, ולכן הגזירה משורשרת בשלושה סבבים — 300,000
אפקטיבי. המשתמשים ב-KV (מרחב `USERS`), לא ב-D1: רשימה קטנה, גישה לפי
מפתח, ואין שאילתות.

Cloudflare Access הוסר — הוא יודע לזהות רק מול ספק זהות או בקוד חד-פעמי,
ולא בסיסמה.

לאימות: `auth-flow-check` (ידני) זורע חשבון, מתחבר אליו באמת, מוודא את
