...
היסטוריה: ה-WAF של מאיה מגביל לפי **נפח בקשות** — חלון של 7 ימים (≈33 עמודים)
עובר והבקשה שאחריו מקבלת 403. לכן האינדקס גדל בשני מסלולים: קדימה בכל ריצת
ניטור (זול), ואחורה ב-`filings-backfill.yml` שרץ בלילה עם תקציב זמן, שומר מצב
אחרי כל חלון וממשיך למחרת. עומק מלא מושג אחרי כמה לילות, לא בהרצה אחת.
