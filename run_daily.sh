...
# --- סקירות הדוחות: **אחרי** שהברייף נכתב ואומת ------------------------
# סקירות הדוחות הכספיים מוכנות מראש לחברות הכיסוי, לבקשת יואב מ-25/08/2026:
# הלחיצה בעמוד מחזירה תוצאה מיידית במקום להמתין לכתיבה.
#
# **הסדר הזה אינו שרירותי.** כשהן רצו לפני הברייף, שתים-עשרה קריאות עם
# PDF מלא אכלו את היתרה — והמוצר עצמו, הברייף, נשאר בלי כלום. נמדד
# ב-28/08: ארבע ריצות רצופות נפלו כך. הברייף הוא המוצר; הסקירות הן
# הכנה מראש למשהו שממילא נוצר בלחיצה, ולכן הן מקבלות את מה שנשאר.
#
# **הסף הוא תקרה ולא יעד.** filing_review מסנן ממילא לדוחות שטרם נסקרו,
# ולכן ביום רגיל הוא מפיק אפס עד כמה; המספר נכנס לפעולה רק בעונת דוחות.
if [ "${REVIEW_LIMIT:-12}" -gt 0 ] 2>/dev/null; then
  BEFORE=${#FAILED[@]}
