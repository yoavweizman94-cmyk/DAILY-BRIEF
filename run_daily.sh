...
if [ -z "${TOPICS_ONLY:-}" ] && [ -z "${BRIEF_FORCE:-}" ] && [ -s "$BRIEF" ]; then
...
if [ -n "${TOPICS_ONLY:-}" ]; then
  # **ריצה שלא כתבה ברייף חייבת לומר זאת.** הדגל יושב בקובץ טריגר
  # שנשמר בגיט, ולכן הוא שורד לטריגר הבא: מי שידחוף טריגר מחר בלי
  # להסיר אותו יקבל ריצה ירוקה שלא הפיקה ברייף. בלוג זה לא נראה.
  echo "::warning title=TOPICS_ONLY — לא נכתב ברייף::הריצה מרעננת חדשות, סיכומי סקטור ואתר בלבד. להסיר את topics_only מ-.trigger/daily-brief.json לפני הטריגר הבא."
else
...
# **הגדר הזה קיים בשביל TOPICS_ONLY.** רענון עמודי הסקטור דורש קריאה
# אחת למודל (summarize_topics), והפקת ברייף דורשת את הקריאה היקרה
# בהרבה שכאן. כשצריך רק לתקן את עמודי הסקטור, אין סיבה לשלם על ברייף
# שכבר נכתב — ובוודאי לא לדרוס אותו.
if [ -z "${TOPICS_ONLY:-}" ]; then
...
גוף** (60-120 מילים) ושורה ריקה לפני האייטם הבא. **אייטם אחד = נושא אחד: אל תאחד שתי \
ידיעות שונות לאייטם אחד עם נקודה-פסיק או 'במקביל'.** אם יש שלוש ידיעות — שלושה אייטמים, \
כל אחד עם כותרת וגוף מלא. מאחדים רק כששתי העובדות הן אותה שרשרת סיבתית. \
ארבעה עד שמונה אייטמים לסעיף, ושישה עד עשרה בנדל\"ן ובנייה. אל תכתוב אייטם ענפי כשורת תבליט אחת מהסוג \
...
fi   # סוף בלוק הפקת הברייף (ראה TOPICS_ONLY למעלה)
...
if [ -n "${TOPICS_ONLY:-}" ]; then
  echo "=== הושלם: רענון עמודי הסקטור (בלי ברייף) ==="
else
