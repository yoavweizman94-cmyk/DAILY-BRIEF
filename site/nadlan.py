...
# מקרא קצר, מתחת לטבלת הערים. שלושת המונחים שאינם מובנים מאליהם גם
# כשהכותרת מלאה — ואחד מהם, פרמיית קבלן, אפשר לקרוא בטעות כרווח היזם.
LEGEND = (
    '<dl class="legend">'
    '<dt>מחיר חציוני למ״ר</dt><dd>חציון של כל העסקאות בשכונות שנסרקו '
    'באותה עיר ברבעון האחרון. חציון ולא ממוצע, כדי שעסקה אחת חריגה '
    'לא תזיז את המספר.</dd>'
    '<dt>שנתי · יד שנייה</dt><dd>אותו שינוי שנתי, מחושב על עסקאות יד '
    'שנייה בלבד. כשהוא רחוק מהשינוי הכולל — מה שזז הוא תמהיל הנמכרות '
    '(יותר דירות חדשות או גדולות) ולא המחיר.</dd>'
    '<dt>פרמיית קבלן</dt><dd>הפער במחיר למ״ר בין דירה מקבלן לדירת יד '
    'שנייה באותה עיר. אינו רווח היזם — דירות חדשות גדולות ומפרטן שונה.</dd>'
    '<dt>חלק מקבלן</dt><dd>שיעור העסקאות שהן רכישה מקבלן, מתוך כלל '
    'העסקאות באותו רבעון.</dd>'
    '<dt>מגמה</dt><dd>המחיר החציוני לאורך הרבעונים שנאספו.</dd>'
    '</dl>')
...
def city_table(st: dict, m: dict) -> str:
    ...
    for key, label in REGIONS:
        ...
        # **כותרת שדורשת פענוח היא כותרת שגויה.** "שנתי", "רבעוני" ו"יד
        # שנייה" זו לצד זו אינן אומרות מה מושווה למה, ו"מצב" אינו אומר
        # מצב של מה. כל כותרת נושאת עכשיו את מה שהיא מודדת ואת בסיס
        # ההשוואה, ומתחת לטבלה יש מקרא לשלושת המונחים שאינם מובנים
        # מאליהם גם כשהכותרת מלאה.
        out.append('<div class="tw"><table class="nadlan"><thead><tr>'
                   '<th>עיר</th><th>מחיר חציוני למ״ר</th>'
                   '<th>שינוי שנתי</th><th>שינוי רבעוני</th>'
                   '<th>שנתי · יד שנייה</th><th>פרמיית קבלן</th>'
                   '<th>עסקאות ברבעון</th><th>חלק מקבלן</th>'
                   '<th>מגמה</th><th>כיוון</th></tr></thead><tbody>')
        for city, v in order:
