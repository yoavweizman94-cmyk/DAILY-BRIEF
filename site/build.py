...
def main() -> int:
    ...
    if briefs:
        ...
        try:
            ...
            _depth = []
        if _depth:
            # **מספר בלי שם הקובץ אינו ניתן לאימות.** ביום שיש בו שלוש
            # מהדורות, המיון בוחר אחת מהן — ומדידה שחוזרת זהה אחרי הפקה
            # חדשה נראית כאילו התיקון לא תפס, בזמן שהיא פשוט נמדדה על
            # קובץ אחר. שם הקובץ מסיר את הספק.
            print(f"::notice::עומק הסעיפים הענפיים ב-{briefs[0].name} "
            ...
            _thin = [f"{n} {w} מילים" for n, c, _, _a, w in _depth if c and w < 55]
            if _few or _thin:
                print("::warning title=סעיפים ענפיים מתחת לרף::"
                      + " · ".join(_few + _thin)
                      + " — הרף: 5 אייטמים בנדל\"ן ובנייה, 3 בשאר, "
                        "ו-55 מילים ומעלה לכל אייטם.")
    ...
    # הנתונים מצנרת commodities-watch; נבנה בכל בנייה, כמו גיליון הרכב.
    _commod = commodities.load()
    (OUT / "commodities.html").write_text(
        PAGE.format(title=f"סחורות · {site_title}", site_title=site_title,
                    root="", body=commodities.page(_commod)),
        encoding="utf-8")
    for _line in commodities.report(_commod):
        print(_line)
